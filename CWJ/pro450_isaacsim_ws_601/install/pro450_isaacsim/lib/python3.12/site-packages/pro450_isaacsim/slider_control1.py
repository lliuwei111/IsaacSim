...
class Slider_Subscriber(Node):
    ...
    def __init__(self):
        ...
        super().__init__("control_slider_fixed")
        ...
        time.sleep(0.05)
        # 夹爪走 485/modbus，必须按 使能 -> 初始化 -> 设开度 顺序，
        # 否则 set_pro_gripper_angle 会静默失败（返回 0 而非抛异常）。
        ret = self.mycobot_450.set_pro_gripper_enabled(1, self.gripper_id)
        self.get_logger().info("set_pro_gripper_enabled(1) -> %s" % ret)
        time.sleep(0.05)
        ret = self.mycobot_450.set_pro_gripper_init(self.gripper_id)
        self.get_logger().info("set_pro_gripper_init() -> %s" % ret)
        time.sleep(1.0)  # init 内部做波特率扫描，耗时较长
        ret = self.mycobot_450.set_pro_gripper_speed(gripper_speed, self.gripper_id)
        self.get_logger().info("set_pro_gripper_speed(%d) -> %s" % (gripper_speed, ret))
    ...
    def listener_callback(self, msg: JointState):
        ...
        ret = self.mycobot_450.set_pro_gripper_angle(gripper_cmd, self.gripper_id)
        self.get_logger().info("gripper_angle: {}, ret: {}".format(gripper_cmd, ret))
